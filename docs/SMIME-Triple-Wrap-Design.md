...
│     └── [encrypted] multipart/signed; protocol="application/pkcs7-signature"
...
1. **Inner sign** — `CAMEL_SMIME_SIGN_CLEARSIGN`, over the composed body part.
   Produces `multipart/signed`. Clear-signing matters here: an opaque
   `signed-data` inner layer is not universally recognised once it is nested
   inside a decrypted envelope. Thunderbird follows a nested `multipart/signed`
   but does not treat a nested opaque `signed-data` part as a signature at all,
   and shows the whole message as unsigned. The triple-wrapped mail that Gmail
   and commercial mail-security gateways produce clear-signs both layers too.
...
| Signature | Covers |
| --- | --- |
| Inner | the original body part: its `Content-*` headers and its body |
...
- Decrypting yields the inner `multipart/signed`, whose signature covers the
